...
#---------------------------------------------------------
col_name_x = ['x'+str(i) for i in range(0,468)]
col_name_y = ['y'+str(i) for i in range(0,468)]
...
while(cap.isOpened()):    
    ...
    if results.multi_face_landmarks: #พบใบหน้าในภาพ
        ...
        if key == ord('n'):
            ...
            faceLms_xy = faceLms_x + faceLms_y + ['normal']
            with open(csv_filename,'a',encoding='UTF8',newline='') as f:
                writer_f = writer(f)
                writer_f.writerow(faceLms_xy)
                f.close()
                print('normal '+str(count_normal))
                
        if key == ord('s'):
            count_shock = count_shock + 1
            faceLms_xy = faceLms_x + faceLms_y + ['shock']
            with open(csv_filename,'a',encoding='UTF8',newline='') as f:
                writer_f = writer(f)
                writer_f.writerow(faceLms_xy)
                f.close()
                print('shock '+str(count_shock))   

        if key == ord('m'):
            count_smile = count_smile + 1
            faceLms_xy = faceLms_x + faceLms_y + ['smile']
            with open(csv_filename,'a',encoding='UTF8',newline='') as f:
                writer_f = writer(f)
                writer_f.writerow(faceLms_xy)
                f.close()
                print('smile '+str(count_smile))
